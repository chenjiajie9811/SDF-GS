...
def sample_points_with_barycentric(mesh, weights = default_barycentric_weights):
    sample_count = mesh.f.shape[0] * weights.shape[0]
    sampled_points = torch.zeros((sample_count, 3))
    for i in range(weights.shape[0]):
        start = mesh.f.shape[0] * i
        end = mesh.f.shape[0] * (i + 1)
        sampled_points[start : end, :] = \
            mesh.v[mesh.f[:, 0], :] * weights[i][0] + \
            mesh.v[mesh.f[:, 1], :] * weights[i][1] + \
            mesh.v[mesh.f[:, 2], :] * weights[i][2]
